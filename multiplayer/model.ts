...
export class Model<T, K extends Game<T,K>> {
    players: Player<T>[];
    ...
    receiveMetaAction(type: string, player: number, t: number) {
      debugger
    ...
    private updateAuthoritative(until: number) {
      while (this.authoritative.t <= until) {
        ...
        this.authoritative.tick(actions);
      }
    }
    private updatePrediction(until: number, rollback: boolean) {
      ...
      while (this.prediction.t <= until) {
        ...
        this.prediction.tick(currentActions);
      }
    }
  }
